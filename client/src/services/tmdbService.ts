...
function transformPaginatedResponse<T>(response: any): PaginatedResponse<T> {
  // Handle both formats: direct pagination object or nested in data
  ...
  return {
    ...
    total_results: response.total_results || response.totalResults || 0,
  };
}

/**
 * TMDB Service
 * Handles all media-related API calls through the backend
 * This keeps the TMDB API key secure on the server side
 */
export const tmdbService = {
  ...
  async getTVShowsByGenre(genreId: number, page = 1): Promise<PaginatedResponse<TVShow>> {
    const response = await api.get(`/tv/genre/${genreId}?page=${page}`);
    return transformPaginatedResponse<TVShow>(response.data);
  },
